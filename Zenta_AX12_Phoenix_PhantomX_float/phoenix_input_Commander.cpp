...
void Commander::begin(unsigned long baud) {
  ...
  DBGSerial.println("After pinPeripeheral"); DBGSerial.flush();
#endif  
#if 1 //def NOT_SURE_WHY_NEEDED_SOMETIMES
  ...
  DBGSerial.println("After pinPeripeheral"); DBGSerial.flush();
#endif  
  DBGSerial.print("USER: "); DBGSerial.println(USER, DEC);
  pinMode(USER, OUTPUT);
#ifdef CHECK_AND_CONFIG_XBEE
  DBGSerial.println("Try XB BAUD"); DBGSerial.flush();
  while (XBeeSerial.read() != -1)
    ;  // flush anything out...
  DBGSerial.println("After read loop");  
  // First lets see if we have a real short _command time
  delay(15);  // see if we have fast _command mode enabled.
  XBeeSerial.print(F("+++"));
  DBGSerial.println("After print");Serial.flush();
  //XBeeSerial.flush();
  DBGSerial.println("After print/flush"); DBGSerial.flush();
  XBeeSerial.setTimeout(20);  // give a little extra time
  DBGSerial.println("Before Readbytes until"); DBGSerial.flush();
  if (XBeeSerial.readBytesUntil('\r', ab, 10) > 0) {
    // Ok we entered _command mode, lets print out a few things about the XBee
  ...
  DBGSerial.println("failed readbytesuntil"); DBGSerial.flush();
  ...
  XBeeSerial.end();
  DBGSerial.println("Try XB 9600"); DBGSerial.flush();
  XBeeSerial.begin(9600);
  while (XBeeSerial.read() != -1)
    ;  // flush anything out...
  ...
  DBGSerial.println("Try XB Configure"); DBGSerial.flush();
...
#endif
  DBGSerial.println("Try XB END"); DBGSerial.flush();
